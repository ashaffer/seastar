...
typedef std::function<future<>(std::unique_ptr<request>* req, connected_websocket* ws)> future_ws_on_dis_connected;
typedef std::function<future<>(std::unique_ptr<request>* req, connected_websocket* ws, temporary_buffer<char> message)> future_ws_on_message;

class websocket_handler : public httpd::handler_websocket_base {

public:
    websocket_handler() : _on_connection([] (std::unique_ptr<request>*, connected_websocket* ws) { return make_ready_future(); }),
                          _on_message([] (std::unique_ptr<request>*, connected_websocket* ws, temporary_buffer<char> message) { return make_ready_future(); }),
                          _on_disconnection([] (std::unique_ptr<request>*, connected_websocket* ws) { return make_ready_future(); })  {}
    ...
    future<> handle(const sstring &path, connected_websocket ws) override {
        ...
        return do_with(std::move(ws), std::move(input), std::move(output), [this] (connected_websocket &ws,
            ...
            return _on_connection(&ws._request, &ws).then([this, &ws, &input, &output] {
                return repeat([this, &input, &output, &ws] {
                    return input.read().then([this, &ws](temporary_buffer<char> buf){
                        if (!buf)
                            return make_ready_future<bool_class<stop_iteration_tag>>(stop_iteration::yes);
                        return _on_message(&ws._request, &ws, std::move(buf)).then([] {
                            return stop_iteration::no;
                        });
                    });
                });
            });
        }).then([this, &ws] { return _on_disconnection(&ws._request, &ws); });
    }
