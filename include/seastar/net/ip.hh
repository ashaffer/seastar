...
namespace std {
    ...
    struct hash<seastar::net::ipv4_address> {
        size_t operator()(seastar::net::ipv4_address a) const noexcept { return a.ip; }
    };

    template <>
    struct hash<seastar::net::ipv6_address> {
        size_t operator()(const seastar::net::ipv6_address&) const noexcept;
    };
};
...
namespace seastar {
    namespace net {
        ...
        struct l4connid {
            ...
            uint32_t reverse_hash(const rss_config& rss_conf) const {
                forward_hash hash_data;
        ...
        struct ipv4_frag_id {
            struct hash : private std::hash<ipv4_address>, private std::hash<uint16_t>, private std::hash<uint8_t> {
                using h1 = typename std::hash<ipv4_address>;
                using h2 = typename std::hash<uint16_t>;
                using h3 = typename std::hash<uint8_t>;
        ...
        template <typename InetTraits>
        struct l4connid<InetTraits>::connid_hash : private std::hash<l4connid<InetTraits>::ipaddr>, private std::hash<uint16_t> {
            size_t operator()(const l4connid<InetTraits>& id) const noexcept {
                using h1 = std::hash<l4connid<InetTraits>::ipaddr>;
