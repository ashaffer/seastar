...
namespace seastar {
    FastClock::time_point FastClock::now () noexcept {
        static thread_local std::chrono::nanoseconds startup{};
        static thread_local uint64_t startup_ticks = 0;

        asm("mfence");
        uint64_t ticks = __rdtsc();

        if (startup_ticks == 0) {
           startup = std::chrono::steady_clock::now().time_since_epoch();
           startup_ticks = ticks;
        }

        uint64_t hz = rte_get_tsc_hz();
        uint64_t ns = ((ticks - startup_ticks) * 1e9) / hz;
        return time_point(startup + std::chrono::nanoseconds(ns));
    }
    ...
    private:
        ...
        explicit reactor_backend_selector () : reactor_backend_selector{default_backend()._name} {}
    ...
    reactor::reactor(unsigned id, reactor_backend_selector rbs, reactor_config cfg)
        ...
        printf("task queue back: 0x%lx, 0x%lx\n", (uint64_t)std::addressof(_task_queues.back().get()->_q), (uint64_t)std::addressof(_at_destroy_tasks->_q));
    ...
    reactor::~reactor() {
        ...
        sigemptyset(&mask);
        sigaddset(&mask, cpu_stall_detector::signal_number());
        [[maybe_unused]] auto r = ::pthread_sigmask(SIG_BLOCK, &mask, NULL);
        assert(r == 0);
        ...
        for (auto&& tq : _task_queues) {
            if (tq) {
                ...
                *internal::current_scheduling_group_ptr() = scheduling_group(tq->_id);
                for (std::size_t i = 0,  sz = _scheduling_group_key_configs.size(); i < sz; ++i) {
                    void* val = tq->_scheduling_group_specific_vals[i];
                    if (val) {
                        if (_scheduling_group_key_configs[i].destructor) {
                            _scheduling_group_key_configs[i].destructor(val);
                        }
                        free(val);
                        tq->_scheduling_group_specific_vals[i] = nullptr;
                    }
                }
            }
        }
    ...
    int reactor::run() {
        ...
        [[maybe_unused]] auto r = sigaction(cpu_stall_detector::signal_number(), &sa_block_notifier, nullptr);
        assert(r == 0);
    ...
    void writeable_eventfd::signal(size_t count) {
        uint64_t c = count;
        [[maybe_unused]] auto r = _fd.write(&c, sizeof(c));
        assert(r == sizeof(c));
    }
    ...
    future<size_t> readable_eventfd::wait() {
        return engine().readable(*_fd._s).then([this] {
            uint64_t count;
            [[maybe_unused]] int r = ::read(_fd.get_fd(), &count, sizeof(count));
...
namespace seastar {
    ...
    std::optional<std::latch> smp::_all_event_loops_done;
    ...
    void smp::allocate_reactor(unsigned id, reactor_backend_selector rbs, reactor_config cfg) {
        ...
        // uses local_engine
        void *buf;
        [[maybe_unused]] int r = posix_memalign(&buf, cache_line_size, sizeof(reactor));
        assert(r == 0);
    ...
    public:
        ...
        auto device_ids() {
            ...
            for (auto it : _mountpoints) {
                devs.push_back(it.first);
            }
            
            return devs;
            // return transform_view{_mountpoints, extract_nth<0>{}};
       }
    };
    ...
    void smp::configure(boost::program_options::variables_map configuration, reactor_config reactor_cfg)
    {
        ...
        using T = typename cgroup::cpuset::value_type;
        for (T t = 0; t < nr_cpus; ++t) {
            cpu_set.insert(t);
        } 
    ...
    static unsigned allocate_scheduling_group_id() {
    ...
    static unsigned long allocate_scheduling_group_specific_key() {
        return  s_next_scheduling_group_specific_key.fetch_add(1, std::memory_order_relaxed);
    }

    static void deallocate_scheduling_group_id(unsigned id) {
    ...
    void reactor::no_such_scheduling_group(scheduling_group sg) {
        throw std::invalid_argument(std::format("The scheduling group does not exist ({})", sg._id));
    }

    const sstring& scheduling_group::name() const {
        return engine()._task_queues[_id]->_name;
    }

    void scheduling_group::set_shares(float shares) {
    ...
    namespace internal {
        inline FastClock::duration timeval_to_duration(::timeval tv) {
