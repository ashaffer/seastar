...
namespace net {
    ...
    std::string config_keys[]{ "mac-address", "pci-address", "port-index", "ip", "gateway", "netmask", "dhcp", "lro", "tso", "ufo", "hw-fc", "event-index", "csum-offload","ring-size" };
...
struct convert<seastar::net::device_config> {
    ...
    decode(const Node& node, seastar::net::device_config& dev_cfg) {
        ...
        for (auto&& item : node) {
            if (none_of(seastar::net::config_keys, [&item](std::string s) {
                ...
                throw seastar::net::config_exception(
                    std::format("unsuppoted key {}", item.first.as<std::string>()));
            }
        }
        ...
        if (node["mac-address"]) {
            dev_cfg.hw_cfg.mac_address = node["mac-address"].as<std::string>();
        }
