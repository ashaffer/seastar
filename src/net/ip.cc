...
compat::optional<l3_protocol::l3packet> ipv4::get_packet() {
    ...
    if (p) {
        ...
        uint delta = std::chrono::duration_cast<std::chrono::microseconds>(now - p->p.getReceivedAt()).count();
        if (delta > 1000) {
...
void icmp::received(packet p, ipaddr from, ipaddr to) {
    ...
    if (_queue_space.try_wait(p.len())) { // drop packets that do not fit the queue
        _inet.get_l2_dst_address(from).then([this, from, to, p = std::move(p)] (ethernet_address e_dst) mutable {
