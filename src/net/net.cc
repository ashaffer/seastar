...
future<> interface::dispatch_packet(packet p) {
    ...
    if (eh) {
        ...
        if (i != _proto_map.end()) {
            ...
            printf("Software hash16: 0x%x 0x%x 0x%x 0x%x\n", (uint32_t)toeplitz_hash16(default_rsskey_40bytes_old, data), (uint32_t)toeplitz_hash16(rss_key(), data), (uint32_t)toeplitz_hash16(default_rsskey_40bytes_old, data), (uint32_t)toeplitz_hash16(rss_key(), data));
